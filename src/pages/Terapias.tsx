...
const Terapias = () => {
  ...
  const terapias = [{
    ...
    detalhes: ["Avaliação e intervenção sensorial", "Desenvolvimento de habilidades motoras finas e grossas", "Treinamento de atividades de vida diária", "Adaptações ambientais e de equipamentos", "Orientação escolar e familiar"],
    cor: "verde-salvia",
    destaque: true
  }, {
    ...
    detalhes: ["Psicoterapia individual para crianças e adolescentes", "Terapia cognitivo-comportamental", "Abordagem lúdica e criativa", "Desenvolvimento de habilidades emocionais", "Acompanhamento de ansiedade e depressão"],
    cor: "azul-sereno",
    destaque: false
  }, {
    ...
    detalhes: ["Orientação parental especializada", "Grupos de apoio para famílias", "Estratégias de manejo comportamental", "Comunicação familiar efetiva", "Rede de apoio e recursos comunitários"],
    cor: "violeta-lavanda",
    destaque: false
  }];
  const planos = [{
  ...
  return <div className="min-h-screen">
      ...
      {/* Hero Section */}
      <section className="pt-32 pb-16 bg-gradient-to-br from-verde-salvia/10 to-bege-suave">
        <div className="container-custom">
          <div className="text-center animate-on-scroll">
            <h1 className="text-azul-confianca mb-6">
              Nossos <span className="text-verde-salvia">Serviços</span>
            </h1>
          ...
          <div className="space-y-16">
            {terapias.map((terapia, index) => <div key={index} className={`grid lg:grid-cols-2 gap-12 items-center ${index % 2 === 1 ? 'lg:grid-flow-col-dense' : ''}`}>
                <div className={`animate-on-scroll ${index % 2 === 1 ? 'lg:col-start-2' : ''}`}>
                  {terapia.destaque && <div className="inline-block px-3 py-1 bg-verde-salvia text-branco-ninho rounded-full text-sm font-medium mb-4">
                ...
                <div className={`animate-on-scroll ${index % 2 === 1 ? 'lg:col-start-1' : ''}`}>
                  <img src={`https://images.unsplash.com/photo-${index === 0 ? '1576091160399-112ba8d25d1f' : index === 1 ? '1573496359142-b8d87734a5a2' : '1609220136736-443140cceacc'}?auto=format&fit=crop&w=800&q=80`} alt={`${terapia.titulo} - Casa Arco Íris`} className="rounded-2xl shadow-lg w-full h-[400px] object-cover" />
                </div>
              </div>)}
          </div>
        </div>
      ...
      <section className="section-padding bg-bege-suave">
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {planos.map((plano, index) => <div key={index} className={`card-custom hover-lift animate-on-scroll ${plano.destaque ? 'ring-2 ring-terracota relative' : ''}`}>
                {plano.destaque && <div className="absolute -top-3 left-6 bg-terracota text-branco-ninho px-4 py-2 rounded-full text-sm font-medium">
                    Mais Procurado
                  </div>}
                ...
                <a href="https://www.doctoralia.com.br" target="_blank" rel="noopener noreferrer" className={`btn-${plano.destaque ? 'primary' : 'secondary'} w-full text-center ${!plano.destaque ? `text-${plano.cor} border-${plano.cor} hover:bg-${plano.cor} hover:text-branco-ninho` : ''}`}>
                  Agendar Consulta
                </a>
              </div>)}
          </div>
