            ...
            {terapias.map((terapia, index) => (
              <div key={index} className={`grid lg:grid-cols-2 gap-12 items-center ${index % 2 === 1 ? 'lg:grid-flow-col-dense' : ''}`}>
                <div className={`animate-on-scroll ${index % 2 === 1 ? 'lg:col-start-2' : ''}`}>
                  {terapia.destaque && (
                    <div className="inline-block px-3 py-1 bg-verde-salvia text-branco-ninho rounded-full text-sm font-medium mb-4">
                ...
                <div className={`animate-on-scroll ${index % 2 === 1 ? 'lg:col-start-1' : ''}`}>
                  <img 
                    src={`https://images.unsplash.com/photo-${index === 0 ? '1576091160399-112ba8d25d1f' : index === 1 ? '1573496359142-b8d87734a5a2' : '1609220136736-443140cceacc'}?auto=format&fit=crop&w=800&q=80`}
        ...
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {planos.map((plano, index) => (
              <div key={index} className={`card-custom hover-lift animate-on-scroll ${plano.destaque ? 'ring-2 ring-terracota relative' : ''}`}>
                {plano.destaque && (
                  <div className="absolute -top-3 left-6 bg-terracota text-branco-ninho px-4 py-2 rounded-full text-sm font-medium">
                    Mais Procurado
                  </div>
                )}
                ...
                <a
                  ...
                  className={`btn-${plano.destaque ? 'primary' : 'secondary'} w-full text-center ${!plano.destaque ? `text-${plano.cor} border-${plano.cor} hover:bg-${plano.cor} hover:text-branco-ninho` : ''}`}
                >
                  Agendar Consulta
                </a>
              </div>
